...
test("account branding, views, and webhooks mirror their public routes", async () => {
  ...
  const deliveries = await client.webhooks.deliveries.list(webhook.id, {
    limit: 5,
  });
...
test("help helper uses canonical GET route with query filters", async () => {
  ...
  const help = await client.help.get({
  ...
  assert.deepEqual(paths(server.requests), [
    "GET /v1/help?topic=runtimes.create&audience=api",
  ]);
});
...
test("runtime helpers use native v1 invocations and files routes", async () => {
  ...
  assert.match(
    String(uploadRequest?.headers["content-type"]),
    /^multipart\/form-data; boundary=/,
  );
  assert.match(
    String(uploadRequest?.body),
...
test("runtime target helpers use live target routes", async () => {
  ...
  const activated = await client.runtimes.targets.activate(
    "runtime_1",
    "target_1",
  );
...
test("browser extension helpers use the cleaned v1 browser extension routes", async () => {
  ...
  const updated = await client.browserExtensions.update(fetched.id, {
    name: "Renamed Wallet",
  });
  ...
  assert.deepEqual(paths(server.requests), [
    "GET /v1/browser-extensions?q=wallet&source=upload&format=crx",
  ...
  assert.equal(uploadRequest?.headers["bctrl-subaccount-id"], "sub_1");
  assert.match(
    String(uploadRequest?.body),
    /name="file"; filename="extension\.crx"/,
  );
  assert.match(String(uploadRequest?.body), /name="name"\r\n\r\nWallet/);
  assert.doesNotMatch(String(uploadRequest?.body), /name="subaccountId"/);
  assert.equal(server.requests[0]?.headers["bctrl-subaccount-id"], "sub_1");
  assert.equal(server.requests[2]?.headers["bctrl-subaccount-id"], "sub_1");
  assert.deepEqual(server.requests[2]?.body, {
    url: "https://chromewebstore.google.com/detail/wallet/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    name: "Wallet import",
  });
...
test("notification recipient helpers use public notification routes", async () => {
  ...
  const listed = await client.notificationRecipients.list({
    type: "email",
    enabled: true,
  });
  const grouped = await client.account.notificationRecipients.list({
    type: "sms",
  });
...
test("invocation live control is runtime-scoped; observability reads are run-scoped", async () => {
  ...
  const waited = await client.runtimes.invocations.wait(
  ...
  assert.equal(waited.waitStatus, "timeout");
  const cancelled = await client.runtimes.invocations.cancel(
    invocation.runtimeId,
    invocation.id,
  );
...
test("hosted tools are created and versioned under tools routes", async () => {
  ...
  const tool = await client.tools.create({
    ...
    source:
      "export default async function handler(ctx) { return { output: ctx.input }; }",
    inputSchema: {
      type: "object",
...
async function createMockServer(): Promise<MockServer> {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (
      method === "POST" &&
      url.pathname === "/v1/webhooks/webhook_1/rotate-secret"
    ) {
      return json(res, 200, {
        id: "webhook_1",
    ...
    if (
      method === "GET" &&
      url.pathname === "/v1/webhooks/webhook_1/deliveries"
    ) {
    ...
    if (method === "POST" && url.pathname === "/v1/runtimes") {
      ...
      return json(
        res,
        201,
        runtimeFixture({ status: "stopped", activeRunId: null }),
      );
    }
    ...
    if (
      method === "POST" &&
      url.pathname === "/v1/runtimes/runtime_1/invocations"
    ) {
      return json(res, 202, {
        id: "invocation_1",
        runId: "run_1",
    ...
    if (
      method === "POST" &&
      url.pathname === "/v1/runtimes/runtime_1/targets"
    ) {
      return json(
        res,
        201,
    ...
    if (
      method === "GET" &&
      url.pathname === "/v1/runtimes/runtime_1/targets/target_1"
    ) {
    ...
    if (
      method === "POST" &&
      url.pathname === "/v1/runtimes/runtime_1/targets/target_1/activate"
    ) {
    ...
    if (
      method === "PATCH" &&
      url.pathname === "/v1/notification-recipients/recipient_1"
    ) {
      return json(
        res,
        200,
    ...
    if (
      ...
      return json(
        res,
        200,
        humanActionFixture({ status: "completed", completedAt: iso() }),
      );
    }
    ...
    if (
      ...
      return json(
        res,
        200,
        humanActionFixture({ status: "cancelled", cancelledAt: iso() }),
      );
    }
    ...
    if (method === "PATCH" && url.pathname === "/v1/browser-extensions/ext_1") {
      return json(
        res,
        200,
        browserExtensionFixture({ name: String(bodyField(body, "name")) }),
      );
    }

    if (
      method === "DELETE" &&
      url.pathname === "/v1/browser-extensions/ext_1"
    ) {
    ...
    if (method === "POST" && url.pathname === "/v1/tools/tool_1/versions") {
      ...
      return json(
        res,
        200,
        toolVersionFixture({ id: "hosted_fn_version_2", version: 2 }),
      );
    }

    if (
      method === "POST" &&
      url.pathname === "/v1/runtimes/runtime_1/files/stage"
    ) {
      return json(res, 200, {
        id: "staged_1",
        runtimeId: "runtime_1",
    ...
    if (
      method === "POST" &&
      url.pathname === "/v1/runtimes/runtime_1/files/upload"
    ) {
      return json(res, 200, {
        id: "staged_1",
        runtimeId: "runtime_1",
    ...
    if (
      method === "POST" &&
      url.pathname === "/v1/runtimes/runtime_1/files/collect"
    ) {
    ...
    if (
      ...
      const waitCount = requests.filter(
        (request) =>
          request.method === "POST" &&
          request.path ===
            "/v1/runtimes/runtime_1/invocations/invocation_1/wait",
      ).length;
      if (waitCount > 1) {
        return json(res, 200, {
          ...invocationFixture({
